...
module ThinkFeelDoDashboard
  module SocialNetworking
    ...
    class ProfileQuestionsController < ApplicationController
      ...
      def index
        @profile_questions = ::SocialNetworking::ProfileQuestion.where(group_id: params[:group_id])
      end

      def create
        @profile_question = @group.profile_questions.build(profile_question_params)
        if @profile_question.save
          redirect_to group_profile_questions_path(@group),
      ...
      def set_profile_question
        @profile_question = ::SocialNetworking::ProfileQuestion.find(params[:id])
      end

      def profile_question_params
        params.require(:profile_question).permit(
